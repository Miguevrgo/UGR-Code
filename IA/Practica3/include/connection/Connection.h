...
// Para control de versiones y obligar a actualizar a versiones incompatibles
// con el online.
...
public:
  ...
  void sendOKRandomPrivateStart(int theplayer, string rival_name,
                                BoardConfig config);
  ...
  static void packet2HelloMaster(Packet &packet, string &ip, int &port,
                                 int &online_version, int &ninja_version);

  static void packet2gameParameters(Packet &packet, int &player, string &name,
                                    BoardConfig &init_board, int &ai_id);
  ...
  static void packet2privateGame(Packet &packet, string &room_name,
                                 string &name);

  static void packet2ninjaStatus(Packet &packet, int &ninja_games,
                                 int &random_games, int &private_games);

  static void packet2OKRandomPrivateStart(Packet &packet, int &my_player,
                                          string &rival_name,
                                          BoardConfig &config);
  ...
  static void packet2move(Packet &packet, int &turn, color &c_piece,
                          int &id_piece, int &dice);
...
struct server_connection {
  ...
  inline bool operator<(const server_connection &b) const {
    return this->ip_addr <
           b.ip_addr; // or this->ip_addr == b.ip_addr and this->port < b.port;
  }
};
...
private:
  ...
  void newNinjaGame(shared_ptr<ParchisServer> server, Packet &packet,
                    shared_ptr<Thread> thread);
  void queueRandomMatchGame(shared_ptr<ParchisServer> server, Packet &packet,
                            shared_ptr<Thread> thread);
  void queuePrivateRoomGame(shared_ptr<ParchisServer> server, Packet &packet,
                            shared_ptr<Thread> thread);
...
/**
 * @brief Servidor maestro que se encarga de gestionar y repartir las conexiones
 * a los servidores ninja.
 *
 */
class MasterServer {
private:
  ...
  void reservePrivateGame(shared_ptr<ParchisServer> server,
