...
def run_level4_test(index, test_id, map_name, params, u075, u15, u25):
    result = subprocess.run(
        [EXEC, f"{MAP_DIR}/{map_name}"] + list(map(str, params)),
        capture_output=True,
        text=True
    ...
    if result.returncode != 0:
        return (index, f"❌ {test_id} Error: {result.stderr.strip()}", "31")

    puntos = parse_level4_points(result.stdout)
    if puntos is None:
        return (index, f"❌ {test_id} No se pudo extraer puntuación", "31")

    nota = calcular_nota_nivel4(puntos, u075, u15, u25)
    color_code ="34" if nota > 2.49 else "31" if nota < 0.75 else "33" if nota < 1.5 else "32"
    return (index, format_score_line(test_id, color_code, nota), None)
...
if __name__ == "__main__":
    ...
    if tests4:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(run_level4_test, idx, *test)
                for idx, test in enumerate(tests4)
            ]

            results = sorted([f.result() for f in concurrent.futures.as_completed(futures)], key=lambda x: x[0])
            for _, line, err_color in results:
                if err_color:
                    print(color_text(line, err_color))
                else:
                    print(line)
                    try:
                        score = float(line.split()[-3])  # Extrae el valor antes de "/ 2.5"
                        level4_scores.append(score)
                    except (ValueError, IndexError):
                        continue

        if level4_scores:
            avg = sum(level4_scores) / len(level4_scores)
            print("-----------")
            print(f"MEDIA NIVEL 4 = {avg:.3f} / 2.5\n")
