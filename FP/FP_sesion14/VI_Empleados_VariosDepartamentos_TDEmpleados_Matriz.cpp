/****************************************************************************
	...
	Se desea organizar los datos de los empleados de una empresa agrupados por 
	departamentos. No todos los departamentos tienen el mismo número de 
	...
	Escriba un programa que lea los datos de los empleados de la empresa y los 
	guarde en una tabla dentada. Después:

	a) Muestre un listado de todos los empleados, por orden de lectura, sin 
	distinción de departamentos.

	b) Muestre un listado de todos los empleados, agrupados por departamentos 
	indicando el número de empleados en cada departamento.

	c) Calcular y mostrar los empleados de menor y mayor sueldo, indicando su 
	departamento.

	d) Calcular y mostrar el sueldo medio, y a continuación, un listado de los 
	empleados con un salario mayor que el sueldo medio y otro con con los 
	empleados con salario menor. Estos dos listados estarán ordenados por 
	apellidos.
...
public:
	...
	// Constructor--> Recibe una secuencia de Empleados que será la primera 
	// fila de la tabla. 
	...
	Empleado GetEmpleado(ParejaEnteros la_pareja){
		return matriz_privada[la_pareja.GetPrimero()][la_pareja.GetSegundo()];
	}
	
	int TotalEmpleados(void){
		int total_empleados;
		for (int departamento = 0;departamento<filas_utilizadas;departamento++){
			total_empleados += num_cols_utilizadas[departamento];
		}
	...
	**************************************************************************/
	ParejaEnteros LocalizacionSalarioMenor (void){
		...
		//Con menor salario
		for (int dpto=0; dpto < filas_utilizadas; dpto++ ){
			for (int emp=0; emp<num_cols_utilizadas[dpto];emp++){
				if (matriz_privada[dpto][emp].GetSalario() < salario_menor){
					salario_menor = matriz_privada[dpto][emp].GetSalario();
					dpto_menor = dpto;
					emp_menor = emp;
				}
			}
	...
	**************************************************************************/
	ParejaEnteros LocalizacionSalarioMayor (void){
		...
		for (int dpto=0; dpto < filas_utilizadas; dpto++ ){
			for (int emp=0; emp<num_cols_utilizadas[dpto];emp++){
				if (matriz_privada[dpto][emp].GetSalario() > salario_mayor){
					salario_mayor = matriz_privada[dpto][emp].GetSalario();
					dpto_mayor = dpto;
					emp_mayor = emp;
				}
			}
	...
	double SalarioMedio (void){
		...
		for (int dpto=0; dpto < filas_utilizadas; dpto++ ){
			for (int emp=0; emp<num_cols_utilizadas[dpto];emp++){
				salario_medio += matriz_privada[dpto][emp].GetSalario();
			}
		}
	...
	SecuenciaEmpleados EmpleadosSalarioMenor (double valor){
		...
		for (int dpto=0; dpto < filas_utilizadas; dpto++ ){
			for (int emp=0; emp<num_cols_utilizadas[dpto];emp++){
				if (matriz_privada[dpto][emp].GetSalario()<valor){
					la_secuencia.Aniade_Elementos(matriz_privada[dpto][emp]);
				}
			}
		}
		return (la_secuencia);
	...
	string ToString_Departamentos(int departamento){
		...
		for (int j=0;j<num_cols_utilizadas[departamento];j++){
			cad += matriz_privada[departamento][j].ToString() + "\n";
		}
		return(cad); 
	}
	string ToString(){
		...
		for (int i=0;i<filas_utilizadas;i++){
			for (int j=0;j<num_cols_utilizadas[i];j++){
				cad += matriz_privada[i][j].ToString() + "\n";
			} 
		}
		return cad;
	}
